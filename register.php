...
if(User::isLoggedin() || User::isUnactivated()){
    ...
    if(isset($username)){
        if(User::usernameExists($username)) $errors[] = 'Korisničko ime već postoji u bazi. Odaberite drugo.';
    ...
    if(isset($email)){
        if(User::emialExists($email)) $errors[] = 'Već postoji korisnik sa ovom E-mail adresom.';
        if(!filter_var($email, FILTER_VALIDATE_EMAIL)) $errors[] = 'Unesite validnu E-mail adresu.';
    }
    
    if(!$errors){
        if(User::registerNewUser($username, $password, $first_name, $last_name, $email)){
